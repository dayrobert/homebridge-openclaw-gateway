...
function resolveApiToken(config, storagePath, log) {
  if (process.env[TOKEN_ENV_VAR]) {
    log.info(`[${PLUGIN_NAME}] Bootstrap token loaded from environment variable ${TOKEN_ENV_VAR}.`);
  ...
  if (existsSync(tokenFilePath)) {
    try {
      ...
      if (fileToken.length >= 16) {
        log.info(`[${PLUGIN_NAME}] Bootstrap token loaded from ${tokenFilePath}.`);
  ...
  if (config.token && config.token.length >= 8) {
    log.info(`[${PLUGIN_NAME}] Bootstrap token loaded from config.json.`);
  ...
  try {
    ...
    log.info(`[${PLUGIN_NAME}] Bootstrap token auto-generated and saved to ${tokenFilePath}.`);
  } catch (err) {
    log.warn(`[${PLUGIN_NAME}] Could not write token file: ${err.message}. Token only in logs.`);
  }

  log.info(`[${PLUGIN_NAME}] ────────────────────────────────────────`);
  log.info(`[${PLUGIN_NAME}] Bootstrap Token: ${generated}`);
