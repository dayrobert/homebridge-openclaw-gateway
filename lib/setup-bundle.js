...
function buildSkillContent(url, token) {
  return `# homekit-events

Poll the homebridge-openclaw plugin for HomeKit state changes and dispatch event-triggered sessions.

## Steps

1. Read the last cursor from \`.claude/homekit-triggers/.cursor\`. If the file does not exist, use a timestamp from 30 minutes ago as the starting point.

2. Fetch the events summary:
   \`\`\`
   GET ${url}/api/events/summary?since=<cursor>
   Authorization: Bearer ${token}
   \`\`\`

3. Write the returned \`cursor\` value to \`.claude/homekit-triggers/.cursor\`.

4. If \`summary\` is empty — output nothing and stop.

5. If \`has_high_priority\` is false — log the summary as a brief note and stop.
...
6. If \`has_high_priority\` is true:
   - Fetch full event detail:
     ...
     Authorization: Bearer ${token}
     \`\`\`
   - Read all \`.md\` files in \`.claude/homekit-triggers/\`
   ...
   - Read \`.claude/homekit-triggers/.cooldowns.json\` (treat as \`{}\` if missing)
   - For each matched trigger: skip if \`now - last_fired < cooldown_minutes * 60000\`
   - For triggers that pass the cooldown check: fire RemoteTrigger with the composed prompt below, then update \`.cooldowns.json\`
...
function buildGarageTriggerContent(url, token) {
  return `---
...
6. Log a one-line summary and your assessment of the situation.
`;
}
...
- Event-specific instructions live in \`.claude/homekit-triggers/\`
- Cursor state: \`.claude/homekit-triggers/.cursor\`
- Cooldown state: \`.claude/homekit-triggers/.cooldowns.json\`

Do not delete files in \`.claude/homekit-triggers/\` — they define how the agent responds to HomeKit events.
