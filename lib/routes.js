...
function setupRoutes(app, apiAuth, uiClient, roomStore, eventQueue, apiConfig = {}) {
  const { externalUrl, bootstrapToken, getEventingStatus } = apiConfig;
  ...
  app.get('/health', async (_req, res) => {
    ...
    res.json({
      ...
      eventing: typeof getEventingStatus === 'function' ? getEventingStatus() : {
        queued_events: eventQueue.count ? eventQueue.count() : undefined,
      },
    });
  });

  app.post('/api/auth/session', apiAuth.requireBootstrap, (req, res) => {
    const sessionToken = apiAuth.issueSessionToken({
    ...
    res.json({
      ...
      expires_in: apiAuth.sessionTtlSeconds,
    });
  });

  // Devices
  app.get('/api/devices', apiAuth.requireSession, async (_req, res) => {
    try {
      const raw = await uiClient.getAccessories();
  ...
  app.get('/api/devices/type/:type', apiAuth.requireSession, async (req, res) => {
    try {
      const raw = await uiClient.getAccessories();
  ...
  app.get('/api/devices/:id', apiAuth.requireSession, async (req, res) => {
    try {
      const raw = await uiClient.getAccessories();
  ...
  // Rooms
  app.get('/api/rooms', apiAuth.requireSession, async (_req, res) => {
    try {
      const raw = await uiClient.getAccessories();
      const devices = parseAccessories(raw, roomStore);
  ...
  app.get('/api/rooms/:room/devices', apiAuth.requireSession, async (req, res) => {
    try {
  ...
  app.post('/api/rooms/learn', apiAuth.requireSession, async (req, res) => {
    const body = req.body || {};
    const assignments = [];
  ...
  app.post('/api/devices/:id/room', apiAuth.requireSession, async (req, res) => {
  ...
  app.delete('/api/devices/:id/room', apiAuth.requireSession, async (req, res) => {
    try {
  ...
  // Device control
  app.post('/api/devices/:id/control', apiAuth.requireSession, async (req, res) => {
  ...
  app.post('/api/devices/control', apiAuth.requireSession, async (req, res) => {
  ...
  // Events
  app.get('/api/events', apiAuth.requireSession, (req, res) => {
  ...
  app.get('/api/events/summary', apiAuth.requireSession, (req, res) => {
  ...
  // Setup bundle
  app.get('/api/setup', apiAuth.requireBootstrapOrSession, (_req, res) => {
    res.json({
      version: '1.0',
      plugin: PLUGIN_NAME,
      auth: {
        bootstrap_endpoint: `${externalUrl}/api/auth/session`,
        session_ttl_seconds: apiAuth.sessionTtlSeconds,
      },
      skills: [{ name: 'homekit-events', path: '.openclaw_gateway/commands/homekit-events.md', content: buildSkillContent(externalUrl, apiAuth.sessionTtlSeconds) }],
      triggers: [{ path: '.openclaw_gateway/homekit-triggers/garage-door-open.md', content: buildGarageTriggerContent(externalUrl, apiAuth.sessionTtlSeconds) }],
      ...
      env: { OPENCLAW_HB_URL: externalUrl, OPENCLAW_HB_BOOTSTRAP_TOKEN: bootstrapToken },
      eventing: typeof getEventingStatus === 'function' ? getEventingStatus() : undefined,
    });
  });
}
