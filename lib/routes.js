...
function setupRoutes(app, apiAuth, uiClient, roomStore, eventQueue, apiConfig = {}) {
  ...
  app.get('/api/setup', apiAuth.requireBootstrapOrSession, (_req, res) => {
    res.json({
      ...
      skills: [{ name: 'homekit-events', path: '.claude/commands/homekit-events.md', content: buildSkillContent(externalUrl, apiAuth.sessionTtlSeconds) }],
      triggers: [{ path: '.claude/homekit-triggers/garage-door-open.md', content: buildGarageTriggerContent(externalUrl, apiAuth.sessionTtlSeconds) }],
